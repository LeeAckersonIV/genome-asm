...
4) when the 2nd and 3rd columns are marked as "DELETE" (upper case), the path is deleted, and the nodes are put in the bucket of unused nodes if they are not used elsewhere. These nodes inherit the assignment in the original path unless a new assignment is provided.
